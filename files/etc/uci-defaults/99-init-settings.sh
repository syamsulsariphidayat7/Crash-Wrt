...
if opkg list-installed 2>/dev/null | grep -q luci-app-amlogic; then
    ...
    if [ -f /usr/bin/hg680pcl ]; then
        ...
        chmod 644 /usr/share/rpcd/acl.d/luci-app-hg680pcls.json
        chmod 644 /usr/lib/lua/luci/controller/hg680pcls.lua
        chmod 644 /usr/lib/lua/luci/model/cbi/hg680pcls.lua
        chmod 644 /usr/lib/lua/luci/view/hg680pcls/controls.htm


        uci -q get hg680pcls.settings > /dev/null || {
            uci set hg680pcls.settings=hg680pcls
            uci set hg680pcls.settings.enabled='1'
            uci set hg680pcls.settings.target_host='104.17.156.243'
            uci set hg680pcls.settings.interval='5'
            uci commit hg680pcls
        }
...
case "$model" in
  *"Amlogic Meson GXL (S905X) P212 Development Board"*)
    echo 'Amlogic STB-HG680P (S905X) - Crash-Wrt' > /tmp/sysinfo/model
    ;;
esac
