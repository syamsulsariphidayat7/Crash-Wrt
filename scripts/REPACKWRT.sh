...
repackwrt() {
    ...
    # Define constants
    local readonly OPHUB_REPO="https://github.com/syamsulsariphidayat7/amlogic-s9xxx-openwrt/archive/refs/heads/main.zip"
    local readonly ULO_REPO="https://github.com/syamsulsariphidayat7/ULO-Builder/archive/refs/heads/main.zip"
    ...
    if [[ "$builder_type" == "--ophub" ]]; then
        ...
        if ! sudo ./remake -b "${target_board}" -u flippy -k "${target_kernel}" -s 1024; then
